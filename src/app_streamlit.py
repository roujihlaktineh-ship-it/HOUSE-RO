...
# Add parent directory to path to access files
...
def load_data():
    import os
    base_path = os.getcwd()
    # Try to load HOUSE_data.csv, fall back to train.csv
    try:
        return pd.read_csv(os.path.join(base_path, 'HOUSE_data.csv'))
    except FileNotFoundError:
        df = pd.read_csv(os.path.join(base_path, 'train.csv'))
        df.rename(columns={'SalePrice': 'HOUSE-PRICES'}, inplace=True)
        df.to_csv(os.path.join(base_path, 'HOUSE_data.csv'), index=False)
        return df
